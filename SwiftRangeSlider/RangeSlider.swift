...
@IBDesignable open class RangeSlider: UIControl {
    ...
    override open func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        // Upgrade the touch space.
        let extraSpace: CGFloat = 10
        
        var lowerKnobFrame = lowerKnob.frame
        lowerKnobFrame = CGRect(x: lowerKnobFrame.origin.x-extraSpace, y: lowerKnobFrame.origin.y-extraSpace, width: lowerKnobFrame.size.width+(extraSpace*2), height: lowerKnobFrame.size.height+(extraSpace*2))
        var upperKnobFrame = upperKnob.frame
        upperKnobFrame = CGRect(x: upperKnobFrame.origin.x-extraSpace, y: upperKnobFrame.origin.y-extraSpace, width: upperKnobFrame.size.width+(extraSpace*2), height: upperKnobFrame.size.height+(extraSpace*2))
        
        if lowerKnobFrame.contains(previousLocation) && upperKnobFrame.contains(previousLocation) {
